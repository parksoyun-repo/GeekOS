...
void Init_VM(struct Boot_Info *bootInfo)
{
	 ...
	 int num_of_pages = bootInfo->memSizeKB >> 2; // 768 3MB내 페이지의 개수. (3M/4K = 768) 
	 int num_of_dir_entries = Round_Up_To_Page(bootInfo->memSizeKB) / PAGE_SIZE;	//1
	Print("%d\n",Round_Up_To_Page(bootInfo->memSizeKB));
	Print("%d\n",(bootInfo->memSizeKB));
	Print("%d\n",PAGE_SIZE);
	//NUM_PAGE_TABLE_ENTRIES : 1024 GEEKOS 소스에서 정해진 최대 페이지 테이블 개수.
	 int i,j;
	struct Page* page;
	...
	{//page directory 부분, i는 페이지 디렉토리의 인덱스이며 해당 index의 page table을 할당역할 
	 	if(i>=num_of_dir_entries+1)
			break;
		kpte = (pte_t*)Alloc_Page();
		memset(kpte, 0, PAGE_SIZE);
		 for(j=0;j<NUM_PAGE_TABLE_ENTRIES ;j++ )
		{//j는 페이지 테이블의 인덱스
			if(i*NUM_PAGE_TABLE_ENTRIES+j==num_of_pages)
				break;

			 kpte[j].present=1;
			 kpte[j].flags=VM_READ|VM_WRITE;//flags를 통해 해당page공간 속성변경
			 kpte[j].pageBaseAddr=(i*NUM_PAGE_TABLE_ENTRIES + j);
			Print("base : %p\n",kpte[j].pageBaseAddr);
			Print("kpte[%d] = %p\n", j, &kpte[j]);
		}
		//Print("kpde[%d] = %p\n", i, &kpde[j]);
